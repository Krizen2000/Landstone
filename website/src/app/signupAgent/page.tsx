...
async function requestAgentSignup(
  firstName: string,
  lastName: string,
  contactNo: string,
  email: string,
  ...
  try {
    res = await axios.post("/api/agents", dataPacket);
  } catch (err) {
    console.error(err);
    return null;
  }
...
export default function SignUp() {
  ...
  const submitAction = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    const value = await requestAgentSignup(
      firstName,
      lastName,
      contactNo,
      email,
  ...
  return (
    <main className="flex flex-wrap justify-center gap-10 py-6 px-4 lg:px-24">
      ...
      <div className="grid gap-5">
        <h1 className="text-center text-4xl font-extrabold">Move Forward!</h1>
        <p className="text-center text-sm">
          We make sure you get the maximum attention from our large customer
          base!
        </p>
        <div className="m-1 px-4 py-8 max-w-md grid gap-1 border-2 border-gray-300 bg-white rounded md:px-8 md:py-12">
          ...
          <Link className="text-sm text-center text-blue-600" href="/signup">
            Are you a Client? Register as Client
          </Link>
